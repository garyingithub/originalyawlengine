...
public class HibernateEngine {
    ...
    private static final Logger logger = LoggerFactory.getLogger(HibernateEngine.class);
    ...
    private void initialise(Set<Class> classes, Properties props)  {
        ...
        try {
            _factory=new MetadataSources(registry).buildMetadata().buildSessionFactory();
    ...
    public boolean exec(Object obj, int action, boolean commit) {
        ...
        try {
            ...
            logger.error("Handled Exception: Error accessing transaction (" +
    ...
    public boolean exec(Object obj, int action, Transaction tx) {
        try {
            ...
            logger.error("Handled Exception: Error persisting object (" +
                    actionToString(action) + "): " + obj.toString(), he);
            if (tx != null) tx.rollback();
            return false;
        }
    ...
    public Transaction getOrBeginTransaction() {
        try {
            ...
            logger.debug("Transaction GET tx = {}; isActive = {}",
                    tx, (tx != null && tx.isActive()));
            return ((tx != null) && tx.isActive()) ? tx : beginTransaction();
        }
        catch (HibernateException he) {
            logger.error("Caught Exception: Error creating or getting transaction", he);
            return null;
        }
    }
    ...
    public List execQuery(String queryString) {
        ...
        try {
            ...
            logger.error("Caught Exception: Couldn't connect to datasource - " +
                    "continuing with an empty dataset");
            if (tx != null) tx.rollback();
        }
        catch (HibernateException he) {
            logger.error("Caught Exception: Error executing query: " + queryString, he);
            if (tx != null) tx.rollback();
        }
    ...
    public Query createQuery(String queryString) {
        ...
        try {
            ...
            logger.error("Caught Exception: Error creating query: " + queryString, he);
            if (tx != null) tx.rollback();
        }
        return null;
    }
    ...
    public void commit() {
        try {
            ...
            if ((tx != null) && tx.isActive()) {
                logger.debug("Transaction COMMIT tx: " + tx);
                tx.commit();
            }
        }
        catch (HibernateException he) {
            logger.error("Caught Exception: Error committing transaction", he);
        }
    }
    ...
    public void rollback() {
        try {
            ...
            logger.error("Caught Exception: Error rolling back transaction", he);
        }
    }
    ...
    public List getObjectsForClassWhere(String className, String whereClause) {
        ...
        try {
            ...
            logger.error("Error reading data for class: " + className, he);
        }
        return result ;
    }
