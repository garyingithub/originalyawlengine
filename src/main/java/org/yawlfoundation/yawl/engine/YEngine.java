...
public class YEngine implements InterfaceADesign,
   ...
   /*****************************************************************************/
    ...
    public String launchCase(String caseID,YSpecificationID specID, String caseParams,
                             URI completionObserver, YLogDataItemList logData)
            throws YStateException, YDataStateException,
            YPersistenceException, YEngineStateException, YQueryException {
        return launchCase(specID, caseParams, completionObserver, caseID, logData, null, false);
    }

    public String launchCase(YSpecificationID specID, String caseParams,
                             URI completionObserver, YLogDataItemList logData,
                             String serviceHandle,String caseID)
            throws YStateException, YDataStateException,
            YPersistenceException, YEngineStateException, YQueryException {
        return launchCase(specID, caseParams, completionObserver, caseID, logData, serviceHandle, false);
    }
        ...
        synchronized(_pmgr) {
            ...
            try {
                if (workItem != null) {
                    ...
                    YIdentifier caseID=workItem.getCaseID().getParent()==null?workItem.getCaseID():workItem.getCaseID().getParent();
    ...
    public YExternalClient getExternalClient(String name) {
        ...
        if (! _externalClients.containsKey("DefaultWorklist")) {
            try {
                addExternalClient(new YExternalClient("DefaultWorklist",
                        PasswordEncryptor.encrypt("resource", null), " admin user"));
            } catch (YPersistenceException e) {
                e.printStackTrace();
            }
        }

        if(name.contains("DefaultWorklist")){
            try {
                addExternalClient(new YExternalClient(name,
                        PasswordEncryptor.encrypt("resource", null), " admin user"));
            } catch (YPersistenceException e) {
                e.printStackTrace();
            }
